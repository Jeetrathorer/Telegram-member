...
async def do_targeted_broadcast(chat_id_bot, data_bc, bot_instance):
    ...
    • Sent/permanently-failed members DB se TURANT delete karo
    ...
    members_per_account = data_bc.get("members_per_account", 5)
...
def cmd_help(msg):
    bot.send_message(
        ...
        "📖 <b>Commands List</b>\n\n"
        "<b>📢 Broadcast:</b>\n"
        "/broadcast — dialog broadcast wizard (media+text+buttons → all dialogs)\n"
        "/quicksend &lt;text&gt; — jaldi text bhejo\n\n"
        "<b>🎯 Scrape + Targeted:</b>\n"
        "/scrape — group members scrape karo\n"
        "/members — scraped members dekho\n"
        "/targeted — scraped members ko DM bhejo\n"
        "/groupadd — contact add → invite link → DM fallback (5/acc)\n"
        "/pollscrape — poll/quiz voters scrape karo (poori history)\n\n"
        "<b>🏷 Tag All:</b>\n"
        "/tagall — ek group ke sab members tag karo\n"
        "/tagallgroups — sabhi groups mein auto tag karo\n\n"
        "<b>📣 Promo:</b>\n"
        "/setpromo — group promo setup karo\n"
        "/promo — sabhi groups mein promote karo\n\n"
        "<b>🤖 Auto-Reply:</b>\n"
        "/autoreply — AI auto-reply ON/OFF\n\n"
        "<b>👤 Misc:</b>\n"
        "/cloneprofile &lt;id&gt; — profile clone\n"
...
def cmd_accounts(msg):
    ...
    if not accs:
        ...
        return
    lines = [f"👤 <b>Accounts ({active} active / {len(accs)} total):</b>\n"]
...
def cmd_add(msg):
    if not is_admin(msg.from_user.id):
        bot.reply_to(msg, "❌ Access denied!"); return
    set_state(msg.from_user.id, "add_phone")
    bot.send_message(
        msg.chat.id,
...
def cmd_members(msg):
    if not is_admin(msg.from_user.id):
        bot.reply_to(msg, "❌ Access denied!"); return
    conn = get_db()
    jobs = conn.execute("SELECT * FROM scrape_jobs WHERE status='done' ORDER BY id DESC LIMIT 10").fetchall()
    conn.close()
    if not jobs:
...
def cmd_targeted(msg):
    ...
    mk.add(InlineKeyboardButton("❌ Cancel", callback_data="cancel_cb"))
    bot.send_message(msg.chat.id, "🎯 <b>Targeted DM ke liye scrape job choose karo:</b>", reply_markup=mk)
...
def cb_bc_job(call):
    bot.answer_callback_query(call.id)
    jid = int(call.data.split("_")[-1])
    set_state(call.from_user.id, "bc_message", {"scrape_id": jid})
    bot.send_message(
        call.message.chat.id,
